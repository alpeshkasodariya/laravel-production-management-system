...
class HomeController extends Controller {
    ...
    public function index() {
        ...
        $total[6]=Order::select('total_kg')->sum('total_kg');
        $total[7]=Order::select('total_price')->sum('total_price');
        $total[8]=Order::select('paid_price')->sum('paid_price');
        $total[9]=Order::select('pending_price')->sum('pending_price');
        ...
        $totalp[6]=Production::select('total_kg')->sum('total_kg');
        $totalp[7]=Production::select('total_price')->sum('total_price'); 
        ...
        $totald[6]=Delivery::select('total_kg')->sum('total_kg');
        $totald[7]=Delivery::select('total_price_pending')->sum('total_price_pending');
     
        $totalpriced=array();
        $totalpriced[0]=$total[0]*170;
        $totalpriced[1]=$total[1]*340;
        $totalpriced[2]=$total[2]*170;
        $totalpriced[3]=$total[3]*340;
        $totalpriced[4]=$total[4]*150;
        $totalpriced[5]=$total[5]*300; 
        ...
        $fullKgd=array();
        $fullKgd[0]=(($total[0]*250) + ($total[1]*500))/ 1000;
        $fullKgd[1]=(($total[2]*250) + ($total[3]*500))/ 1000;
        $fullKgd[2]=(($total[4]*250) + ($total[5]*500))/ 1000;
        ...
        return view('dashboard', compact('total','totalprice','fulltotalprice','fullKg','totalp','totalpricep','fulltotalpricep','fullKgp','totald','totalpriced','fulltotalpriced','fullKgd','kaju','badam'));
    }
